...
Copy over the files to be sent to SeaBASS from the cloud to the local in:
data > raw > hplc
- include 
  - xlsx data files
  - method file
...
* June 2026 to be sent
M-K_13-01c_report
MBON_14-17_report_Sep_2023_Mar_2024
MBON_14-43_report_2024_all
...
cal_file <- 
  ...
  basename() %>%
  str_subset("ignore", negate = TRUE) %T>% 
  print()
...
file_pig <-
  ...
  mutate(
    ...
    cal_files = 
      case_when(
        str_detect(base, "14-17_repor|14-43_report") ~ str_subset(cal_file, "01122026"),
        str_detect(base, "13-01B|13-01c_report") ~ str_subset(cal_file, "10032021"),
...
hplc_data <-
  ...
  mutate(
    cruise_id = case_when(
      ...
      str_detect(cruise, "(?i)hog") ~ str_c("HG", str_extract(cruise, "[0-9]+")),
      str_detect(cruise, "(?i)CMS Collaborative") ~ "CORE1",
      
      .default = "no"
      ),
    
    # fix any cruise names that done match NOAA SFER
    cruise_id = case_when(
      str_detect(cruise_id, "WS21277") ~ "WS21278",
      
      .default = cruise_id
    ),
    cruise = case_when(
      str_detect(cruise, "21277") ~ "Walton Smith 21278",
...
The identifiers come from `hplc_gsfc_id` column

MBON_14-17_report_Sep_2023_Mar_2024
* From Crystal: Some of the filter dropped during sonication and acetone % is 
  slightly lower. Precision with rep filter is not particularly poor, but I 
  still recommend only using this sample qualitatively and NOT reporting it to 
  SeaBASS.
  - 14-1512
  
MBON_14-43_report_2024_all
* labeled as duplicates but don't have similar pigments so will be removed
  - 14-2905
  - 14-2906

M-K_13-01c_report
* Reps don’t look like each other. 
  - 13-2298
  - 13-2299
  - 13-2270
  - 13-2271
...
hplc_data <- 
  ...
  filter(
    # MBON_14-17_report_Sep_2023_Mar_2024 
    !str_detect(hplc_gsfc_id, "14-1512"),
    # MBON_14-43_report_2024_all
    !str_detect(hplc_gsfc_id, "14-2905|14-2906"),
    # M-K_13-01c_report
    !str_detect(hplc_gsfc_id, "13-2298|13-2299|13-2270|13-2271|13-2226|13-2227") 
    ) %T>% 
  print()
```
...
analysis_precise <-
  ...
  pivot_wider(
   ...
   mutate(across(c(t_chl_a, ppig), ~ scales::percent(., 0.01))) %T>% 
  print()
...
hplc_data <-
  hplc_data %>%
  left_join(., nest(precise, .by = base, "precision" = base:n), by = join_by(base)) %>%
  # left_join(., nest(duplicates, .by = base, "analysis_precise" = base:n), by = join_by(base)) %T>%
  print()
...
## Write New Checklist files (.rtf/.docx)
  ...
  mutate(
    text = pmap(
      ...
      (function(y, z, dat) {
        ...
        if (str_detect(z, "rtf")) {
          message(glue("Using `.rtf. file: {y}"))
        x <- readLines(z) %>%
                tibble(text = .)
          x %>%
          mutate(
            text = str_replace(
              text, srch_terms$search[[1]],
              glue("\\0: {srch_terms$info[1]}")
            ),
            text = str_replace(
              text, srch_terms$search[[2]],
              glue("\\0: {y}")
            ),
            text = str_replace(
              text, srch_terms$search[[3]],
              glue("\\0 {srch_terms$info[3]}")
            ),
            text = str_replace(
              text, srch_terms$search[[4]],
              glue("\\0 {srch_terms$info[4]}")
            ),
            text = str_replace(
              text, srch_terms$search[[5]],
              glue("\\0 {if_else(dupes != 0, as.character(dupes),
                                        'no')}")
            ),
            text = str_replace(
              text, srch_terms$search[[6]],
              glue("\\0 {srch_terms$info[6]}")
            ),
            text = str_replace(
              text, srch_terms$search[[7]],
        ...
        message(glue("Using `.docx file: {y}"))
        
        docx_txt <- officer::read_docx(z)

        docx_txt <-
          docx_txt %>%
          officer::body_replace_all_text(
            srch_terms$search[1],
            glue("{srch_terms$search[1]} {srch_terms$info[1]}")
            ) %>%
          officer::body_replace_all_text(
            srch_terms$search[2],
            glue("{srch_terms$search[2]} {y}")
            ) %>%
          officer::body_replace_all_text(
            srch_terms$search[3],
            glue("{srch_terms$search[3]} {srch_terms$info[3]}")
            ) %>%
          officer::body_replace_all_text(
            srch_terms$search[4],
            glue("{srch_terms$search[4]} {srch_terms$info[4]}")
            ) %>%
          officer::body_replace_all_text(
            srch_terms$search[5],
            glue("{srch_terms$search[5]} {if_else(dupes != 0, as.character(dupes), 'no')}")
            ) %>%
          officer::body_replace_all_text(
            srch_terms$search[6],
            glue("{srch_terms$search[6]} {srch_terms$info[6]}")
            ) %>%
          officer::body_replace_all_text(
            srch_terms$search[7],
            glue("{srch_terms$search[7]} {srch_terms$info[7]}")
            ) 
              
        print(docx_txt, target = str_replace(z, "\\.docx", ".docx"))
        }
      })
    )
  )
...
for (i in seq(nrow(file_pig))) {
  ...
  temp <-
    ...
    unnest(comm) %>%
    mutate(
      comm = str_remove(comm, "\\[Thread[\\s\\S]*Comment:\\s*"),
      comm = str_replace_all(comm, "\\n", " ")
      ) %T>% 
    print()
...
xlsx_comm %>%
  ...
  ) %>%
  {
    if (FALSE) {
      ...
      message("Not saving comments file")
      .
    }
  }
```
