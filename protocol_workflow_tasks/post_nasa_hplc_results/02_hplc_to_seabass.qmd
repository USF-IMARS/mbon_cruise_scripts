...
## 3.1 ---- Edit Header Information Here -----------------------
...
acknowledge <-
  str_c("! ",
    ...
    c(
    "ACKNOWLEDGMENTS",
    "Observations were contributed by the South Florida Marine Biodiversity Observation",
    "Network (SFMBON), the Southeast US Marine Biodiversity Observation Network",
    "(SEUSMBON), and the FRESCA project (Characterizing and forecasting coastal",
    "ecosystem responses to multiple stressors for management applications in",
    "South Florida). This research was funded by the National Oceanic and",
    "Atmospheric Administration (NOAA grants No. NA19NOS0120199;",
    "NA23NOS4780271-T1-01); Gulf of America Coastal Ocean Observing System",
    "(NOAA-GCOOS Cooperative Agreement No. NA16NOS0120018); and the National",
    "Aeronautics and Space Administration (NASA grants No. NNX14AP62A,",
    "80NSSC20K0017, and 80NSSC22K1779)."
    ),
    collapse = "\n"
  ) %>%
...
## version notes:
## if version is R2 or above, add version notes if need to resubmit
## general application:
...
* description - describes column

```{r field-names-from-seabass}
shelf(rvest)
field_names <- read_html("https://seabass.gsfc.nasa.gov/wiki/stdfields") %>%
  html_table()

field_names[[1]] # field names
field_names[[2]] # suffix
field_names[[3]] # modifiers

if (FALSE) {
  field_names <-
    here("data", "metadata", "seabass_standard_fields_all.txt") %>%
    read_tsv(
...
# find overlap between names from HPLC data and current SeaBASS names
sb_names <-
  hplc_data %>%
  pull(names) %>%
  bind_rows() %>%
  distinct() %>%
  left_join(
    sb_field, by = c("param_name" = "low_name")
  ) %T>% 
  print()
```
...
hplc_data <- hplc_data2 <-
  ...
  mutate(
    ...
    #         str_detect(low_name, "tchl_a") ~ str_replace(
    #           low_name,
    #           "tchl_a", "tchla"
    #         ),
    #         str_detect(low_name, "^t_chl") ~ str_replace(
    #           low_name,
    #           "t_chl", "tchl"
    #         ),
    #         str_detect(low_name, "t_acc") ~ str_replace(
    #           low_name,
    #           "t_acc", "tacc"
    #         ),
    #         .default = low_name
    ...
    sb_name = map(
      ...
      \(x) {
        x %>%
        left_join(.env$sb_names, by = c("param_name"))
        }
      ...
      function(x, y) {
        ...
        if (ncol(x) > 2) {
          rep_names <-
            ...
            mutate(
              units = if_else(
                str_detect(param_name, "bincount"), 
                "none", 
                units
                )
            )
...
# 4.0 Saving Files in SeaBass Format
...
Needs `sb_data` from another file?
```{r date-range}
if (FALSE) {
  arg_ls <-
    list(
...
arg_ls <- 
  list(
    ...
    cloud_loc = here("data", "processed", "hplc", "info_by_cruise"), # for testing of experimental change
...
if (!arg_ls$wrt_cld) {
  ...
  cli_alert_warning(
    ...
    cli_text("\n\n")
    cli_alert_warning(c("Save location: ", arg_ls$cloud_loc))
    ...
    reduce(
      ...
      list(
        ...
        tibble(sb_file = .) %>%
        mutate(
          cruise_id = str_extract(sb_file, "((WS|WB|SV|HG|H|CORE)[0-9].*)(?=_p)")
          ),
        ...
        here("data", "raw", "hplc", "checklist") %>%
          ...
          tibble(chk_file = .) %>%
        mutate(
          cruise_id = str_extract(chk_file, "((WS|WB|SV|HG|H|CORE)[0-9].*)(?=\\.)")
          ),
        ...
        # ---- extract path to map files
         ...
         tibble(map_file = .) %>%
         mutate(
           cruise_id = str_extract(map_file, "((WS|WB|SV|HG|H|CORE)[0-9].*)(?=_s)")
           )
        )
      ) %>%
...
## 4.2 Save Consolidated Info and Select Variables
