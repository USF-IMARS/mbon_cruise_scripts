...
cruise_id <-
  tibble(
    ship        = "WB",                                   # HERE, ship acronym
    start_dates = "June 17 2026",         # START DATE HERE, month day year
    end_dates   = "June 21 2026",           # END DATE HERE, month day year
...
loca <-
  ...
  filter(is.na(other) | (str_detect(other, "Cross|Red") | str_detect(station, "^(4|13|24)$"))) %T>%
  print()
...
if (str_extract(dir_path, "\\d{4}") >= 2026) {
  # update 2026, to use rows 5 - 10 to get lat, lon, labs
  zoo_loc <-
    readxl::read_excel(
      dir_path,
      sheet = "zooplankton",
      range = cellranger::cell_rows(c(5, 10)),
      .name_repair = janitor::make_clean_names
    ) %>%
    select(station, lat, lon, labs) %T>% 
    print()
} else {
  # prior to 2026
  zoo_loc <-
    ...
    mutate(
      ...
      labs = case_when(
        str_detect(labs, "500") & str_detect(labs, "64") ~ "64, 200, 500",
        str_detect(labs, "64") ~ "64 (only)",
        .default = labs
      ),
      labs = case_when(
        str_detect(station, "MR|LK|WS") ~ "64, 200, 500",
        str_detect(labs, "64") ~ "64 (only)",
        .default = labs
      )
    )
...
noaa_loc <-
  ...
  mutate(
    ...
    lon = if_else(is.na(lon), longitude_decimal_degree, lon),
    lat = if_else(is.na(lat), latitude_decimal_degree, lat),
    .before = 2
    )
...
```{r data-manipulation}
# include L line?
if (!askYesNo("Include L Line")) {
  loca <-
    loca %>%
    filter(!str_detect(station, "^L\\d{1,2}")) %T>% 
    print()
}
...
loca <-
  loca %>%
    mutate(
      ...
      labl = if_else(str_detect(station, paste0(paste0("BG", c(seq(9, 17, 2), 20)), collapse = "|")),
                      "CHL, HPLC, CDOM, eDNA ", labl),
      labl = if_else(str_detect(station, "^(4|13|24)$"), "Optional", labl),
      labl = if_else(str_detect(labl, "Cross"), NA, labl)
    ) %>%
  filter(!is.na(labl)) %>%
  mutate(
    .by = labl,
...
if (exists("base_plt")) {
  ...
  world_download(
    path_land  = map_loc,
    path_topo  = here(map_loc, "flracepv"),
    extent     = exnt,
    use_suffix = NULL
  )
  ...
  map_obj <-
    load_map_obj(
      .map_coast = map_loc,
      .map_state = NULL,
      .map_bath  = here(map_loc, "flracepv"),
      .map_file  = "etopo1.nc",
      .extent    = exnt
    )
...
loca_edna <-
  ...
  mutate(
    ...
    depth_level =
      case_when(
        str_detect(station, "BG9|BG11") ~ "surf and bot",
        str_detect(station, "BG1[357]|BG20") ~ "surf, DCM, bot",
        .default = depth_level
      ),
  ) %>%
  ...
  mutate(
    .by = depth_level,
    labl = glue("{edna} {depth_level} ({n()})", .na = "")) %T>% 
  print()
...
imars_map <-
  ...
  theme(
    legend.position        = "inside",
    legend.position.inside = c(0.77, 0.18),
    legend.text            = element_text(size = 10),
    legend.title           = element_text(size = 15),
    legend.background      = element_rect(color = "black")
  )
