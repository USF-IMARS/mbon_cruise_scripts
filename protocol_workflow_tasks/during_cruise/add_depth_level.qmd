...
# Samples Collected for NOAA
```{r}
cruise_path <- 
  here("C:/Users", "imars_cruise", "Documents", "s_fl_cruise_data") %>% 
  dir_ls(regexp  = "07_2023_WS23203",
         recurse = TRUE) %>% 
  str_subset("~", negate = TRUE) %T>% 
  print()

if (length(cruise_path) > 1) {
  cruise_path <- cruise_path[menu(cruise_path)]
}

# Chla, HPLC, CDOM samples
sample_collect <- 
  (readxl::excel_sheets(cruise_path))[1] %>% 
  read_excel(
    path  = cruise_path,
    sheet = .,
...
# eDNA ----
edna_collect <- 
  (readxl::excel_sheets(cruise_path))[3] %>% 
  read_excel(
    path  = cruise_path,
    sheet = .,
    .name_repair = janitor::make_clean_names
...
```{r save-collected}
cruise_id <-
  pull(sample_collect, collected)[1] %>% 
...
if (!file_exists(file_name) | FALSE) {
  full_join(
    sample_collect,
    select(edna_collect, collected, eDNA)
    ) %>% 
  openxlsx::write.xlsx(file = file_name)
