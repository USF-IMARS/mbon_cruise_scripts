...
water_body <- list(
  ...
  "West Florida Shelf" = c(
    ...
    "REDTIDE", "BloomHG26045", "PN_1", "ROGE5"
  )
) %T>% print()
```
...
distinct(meta_format, station, name.of.water.body) %T>% 
  {
    filter(., is.na(name.of.water.body)) %>%
    print()
  }
```
...
getSheetNames(prev_f)[-c(1, 5:12)] %>%
  str_subset("(HG|WS)(25|26)")   # <--- edit here to select which sheets to use
