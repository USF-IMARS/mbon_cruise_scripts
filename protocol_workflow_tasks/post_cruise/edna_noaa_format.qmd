...
noaa_format <-
  ...
  mutate(
    across(is.character, ~ if_else(.x == "0", NA, .x)),
    across(is.numeric, ~ if_else(.x == 0, NA, .x)),
    sample_name = str_replace(sample_name, "_0?_", "_BLANK_")
    ) %T>%
  print() %>%
  
  # final select of names
  select(
    ...
    sample_replicate,
    "samp_collect_notes"   = notes,
    "decimalLatitude"      = lat,
    "decimalLongitude"     = lon,
    geo_loc_name,
    "eventDate",
    "verbatimEventDate"    = mm_dd_yy,
    "verbatimEventTime"    = time_gmt,
    "samp_size"            = vol_ml,
    samp_size_unit,
    "depth_category"       = depth_level,
    "minimumDepthInMeters" = depth_m,
    "maximumDepthInMeters" = depth_m,
    "tot_depth_water_col"  = max_depth_m,
  ) %T>%
  print()
...
file_name <- 
  here(
    local_dir,
    glue("NOAA_eDNA_metadata_{cruise_name}B.xlsx")
    )

# load template
wb <- wb_load(noaa_template, sheet = "noaa_format")

# remove extra sheets (NOT noaa_format)
for (i in wb_get_sheet_names(wb)) {
  if (str_detect(i, "noaa_format")) next
  message(paste("Removing Sheet:", i))
  wb <- wb_remove_worksheet(wb, sheet = i)
}

# add data
wb <- wb_add_data(
  wb, x = noaa_format, sheet = "noaa_format", na.strings = "",
  col_names = FALSE, start_row = 2)

# format verbatimEventDate to mm/dd/yyyy
wb <- wb_add_numfmt(
  wb, 
  dims = wb_dims(x = noaa_format, cols = "verbatimEventDate"), 
  numfmt = "mm/dd/yyyy"
  )
