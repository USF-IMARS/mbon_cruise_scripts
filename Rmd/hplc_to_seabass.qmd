...
## 2.2 ---- Edit Header Information Here -----------------------
```{r edit-chunk}
prepared_by <- 
  list(
...
## 2.3 All SeaBass Standard Fields 
...
# 3.0 Read SeaBass HPLC Files and Check for Issues
...
# check if Perl is installed and errors if not
tryCatch({
  system2("perl", "-v", stdout = TRUE)
  invisible()
  }, error = function(e) {
    stop(glue("The programming language \"Perl\" does not seem to be ", 
...
# this will download fcheck if it doens't exists in the project directory
# <https://seabass.gsfc.nasa.gov/wiki/FCHECK/fcheck4.tar>
if (!dir_exists(here("fcheck")) | !file_exists(here("fcheck", "fcheck4.pl"))) {
  (function() {
  ...
  url  <- "https://seabass.gsfc.nasa.gov/wiki/FCHECK/fcheck4.tar"
   ...
   message(
    glue_col(
      ...
      "If you type <ENTER> in consol, this will create the repo in ",
      "{blue {here()}}\n",
   ...
   if (x != 2) return(message("Not downloading `fcheck`"))
   
 
  message("Creating `fcheck` folder and downloading `fcheck.pl`")
    
  dir_create(here("fcheck"))
  file <- here("fcheck", basename(url))
  
  # download
  message(glue_col("Downloading to {blue {here(\"fcheck\")}}"))
  download.file(
    url      = url,
    destfile = file,
    method   = "curl")
  ...
  # unzip
  message(glue_col("Unzipping to {blue {here(\"fcheck\")}}"))
  untar(
    tarfile = file,
    exdir   = here("fcheck"))
  ...
  print(
    with(
      ...
      glue(basename(files[[i]]),
           fcheck2[[i]][which(str_detect(fcheck2[[i]], 
                                         "\\d+.*error"))],
           .sep = ": ")
      )
    )
    ...
    reduce(
      ...
      list(
        ...
        # ---- extract path to map files
         ...
         tibble(map_file = .) %>%
         mutate(
           cruise_id = str_extract(map_file, "((WS|WB|SV)[0-9].*)(?=_s)")
           )
        )
      ) %>%
...
If want to save a the consolidated HPLC data with the selected variables as a 
`.csv` file.
```{r sv-consolidated}
arg_ls <- 
  list(
