...
hd_def$comments <- 
  glue(.sep = "\n",
       ...
       "! COMMENTS",
       "!",
       "! Measurements below detection limits are assigned the value -8888",
       "! ",
...
xl <- hplc_data$precision[[5]]
xl <- hplc_data$precision[[1]]

for (i in 1:25) {
  xl <- hplc_data$precision[[i]]
  cat(i, hplc_data$cruise_id[[i]], sep = "\n")
  
  if (any(str_detect(xl, "replicate"), na.rm = TRUE)) {
    glue("!") %T>% print()
  } else if (nrow(xl) > 1) {
    
    glue("!Coefficient of variation (replicate filter precision) for",
...
pss <- "yes"
glue("{if_else(str_detect(xl$x, 'rep'), 'yes{pss}', 'false')}")

# include?
#!/experiment=SFP
#!/experiment=Sanctuaries_Marine_Biodiversity_Observation_Network_(MBON)

# hd_def$expr_ex1 = "SFP"
# hd_def$expr_ex2 = "Sanctuaries_Marine_Biodiversity_Observation_Network_(MBON)"
```
...
header_skeleton <-
  glue::glue(
    ...
    #!/experiment={hd_def$expr_ex1} # <---- include?
    #!/experiment={hd_def$expr_ex2} # <---- include?
    "/cruise={cruise_id}",
    "/station={hd_def$stn}",
