...
pwalk(list(final$cols, final$heads),
      function(x, y) {
        ...
        # fcheck
        f <- file(here(loc, y$file_name), open = "wb") 
        cat(y$header, "\n", sep = "",
...
## FCHECK: Checking `.sb` Files Locally
...
if (!dir_exists(here("fcheck")) | !file_exists(here("fcheck", "fcheck4.pl"))) {
  ...
  file <- here("fcheck3", basename(url))
...
# cloud_loc <- here("data", "test") # for testing if made experimental change
...
if (!wrt_cld) {
  ...
  cli_alert_warning(c("If you want to save files to {.file {cloud_dir}}, ",
    ...
    meaning <-
      if_else(ovrwrt,
              glue(" meaning you will overwrite the data in the cloud ", 
                   "directory, but if using box, it will be version ", 
                   "controlled."),
              " meaning you will not be overwriting data in the cloud.")  
    
  cli_text("\n\n")
  cli_alert_warning("You might be overwriting previsouly saved data.")
  cli_alert_warning(c("Currently, {col_red(\"ovrwrt\")} = ",
                      "{.var {col_red(ovrwrt)}}{meaning}"))
