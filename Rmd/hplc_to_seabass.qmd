...
field_unit <- 
  ...
  distinct(param_name) %>%
  arrange(param_name) %>%
  mutate(low_name = str_to_lower(param_name),
         low_name = str_replace(low_name, "(?i)caro", "car"),
         low_name = case_when(
           str_detect(low_name, "chl_c12") ~ "chl_c1c2",
           str_detect(low_name, "ppc_tpig") ~ "ppc_tpg",
           str_detect(low_name, "tchl_a_tpig|tchl_a_tpg") ~ "tchla_tpg",
           .default = low_name
         )) %>%
...
```{r header-info}
# pig_w_head$data[[1]] %>% janitor::remove_empty(which = "cols") %>%
#   as.matrix() 
    ...
    cruise_id = case_when(
     ...
     heads = pmap(
      ...
      function(y, x, cruise_id){
        ...
        field_nm <- 
          tibble(param_name = y$param_name[[1]]) %>%
          left_join(., !!field_unit, by = "param_name") %T>% print() 
        
        fields <- glue(!!!fields_extr$field_extr,
                       paste(field_nm$field_name, collapse = ","), .sep = ",")
        
        pun    <- glue(!!!fields_extr$units,
                       paste(field_nm$units, collapse = ","), .sep = ",")
        
        header <- 
          with(x, eval(header_skeleton)) %T>% 
          print()
        ...
        return(
          list(header    = header,
               file_name = file_name,
               fld_use   = paste(c(fields_extr$field_extr, field_nm$field_name)),
               fields    = field_nm)
          )
      }
     )
...
final1 <- 
  ...
  slice(1) %>%
  mutate(
    cols = map2(.x = data, .y = heads, 
               function(.x, .y) {
                 select(.x, any_of(c(fields_extr$field_extr, 
                                     final$heads[[1]]$fields$param_name))) %>%
                 set_names(.y$fld_use)
 ...
 final1 %>%
  mutate(
    cols2 = map2(.x = cols, .y = heads,
                function(.x, .y) {
                  # print(names(.y))
                  #print((.y$fields$field_name))
                  # print(.x)
                  
                  cat(.y$header, "\n", #delim = ",",
                      file = here(.y$file_name))
                  .x %>%
                   mutate(
...
final$heads[[1]]$file_name
#[1] "hplc_gsfc_id" "sample"       "date"         "station"      "lon"         
# [6] "lat"          "depth"        "water_depth"  "volfilt"  
...
test <- 
  tibble(
    head = c("hplc_gsfc_id", "sample", "date", "time", "station", "lon", "lat", "depth", "water_depth", "volfilt", "Tot_Chl_a", "Tot_Chl_b", "Tot_Chl_c", "alpha-beta-Car", "But-fuco", "Hex-fuco", "Allo", "Diadino", "Diato", "Fuco", "Perid", "Zea", "MV_Chl_a", "DV_Chl_a", "Chlide_a", "MV_Chl_b", "DV_Chl_b", "Chl_c1c2", "Chl_c3", "Lut", "Neo", "Viola", "Phytin_a", "Phide_a", "Pras", "Gyro", "Tchl", "PPC", "PSC", "PSP", "Tcar", "Tacc", "Tpg", "DP", "Tacc_Tchla", "PSC_Tcar", "PPC_Tcar", "TChl_Tcar", "PPC_Tpg", "PSP_Tpg", "Tchla_Tpg", "NA")
,


    body = c("hplc_gsfc_id", "sample", "date", "time", "station", "lon", "lat", "depth", "water_depth", "volfilt", "Tot_Chl_a", "Tot_Chl_b", "Tot_Chl_c", "alpha-beta-Car", "But-fuco", "Hex-fuco", "Allo", "Diadino", "Diato", "Fuco", "Perid", "Zea", "MV_Chl_a", "DV_Chl_a", "Chlide_a", "MV_Chl_b", "DV_Chl_b", "Chl_c1c2", "Chl_c3", "Lut", "Neo", "Viola", "Phytin_a", "Phide_a", "Pras", "Gyro", "Tchl", "PPC", "PSC", "PSP", "Tcar", "Tacc", "Tpg", "DP", "Tacc_Tchla", "PSC_Tcar", "PPC_Tcar", "TChl_Tcar", "PPC_Tpg", "PSP_Tpg", "Tchla_Tpg", "NA")


) 

identical(test[1,1][[1]], test[1,2][[1]])
test %>%
mutate(
  check = if_else(head == body, "yes", "")
)
```
