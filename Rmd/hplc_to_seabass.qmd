...
pigment_data <- hplc_data
# ocation to save files
loc <- 
  here("data", "processed", "hplc", "ind_file")
    ...
    sb_name = map(
      ...
      (\(x) 
         ...
         low_name = case_when(
           ...
           # str_detect(low_name, "t_pg|t_pig") ~ str_replace(low_name, "t_pg|t_pig", "tpg"),
...
field_unit$sb_name %>%
  ...
  #          str_detect(low_name, "tchl_a") ~ str_replace(low_name, "tchl_a", "tchla"),
  #          str_detect(low_name, "t_pg") ~ "tpg",
  #          str_detect(low_name, "t_chl") ~ "tchl",
  #          str_detect(low_name, "t_acc") ~ "tacc",
  #          .default = low_name)
  #        ) %>%
  # left_join(sb_field, by = c("low_name")) 
...
# fields 
# has seabass version and original version to map to
...
(c("hplc_gsfc_id,sample,date,time,station,lon,lat,depth,water_depth,volfilt,Tot_Chl_a,Tot_Chl_b,Tot_Chl_c,alpha-beta-Car,But-fuco,Hex-fuco,Allo,Diadino,Diato,Fuco,Perid,Zea,MV_Chl_a,DV_Chl_a,Chlide_a,MV_Chl_b,DV_Chl_b,Chl_c1c2,Chl_c3,Lut,Neo,Viola,Phytin_a,Phide_a,Pras,Gyro,Tchl,PPC,PSC,PSP,Tcar,Tacc,Tpg,DP,Tacc_Tchla,PSC_Tcar,PPC_Tcar,TChl_Tcar,PPC_Tpg,PSP_Tpg,Tchla_Tpg,NA,NA") %>%
  str_split(",", simplify = TRUE) %>%
  as.vector()) %>%
  duplicated


names(pigment_data[[2]][[5]])[-c(1:15)][-c(42, 43)]
(pigment_data[[2]][[5]]$param_name)[[1]]
  ...
  mutate(
     heads = pmap(
      ...
      function(y, x, cruise_id, sb_name){
        ...
        file_name <-
          glue("{expri}", "{cruise_id}", "{type}", "{x$start_date}.sb", 
               .sep = "_")
        
        # fields and units standards: https://seabass.gsfc.nasa.gov/wiki/stdfields
        # SeaBass standard fields
        field_nm <- c(fields_extr$field_extr, sb_name$field_name)
        fields <- 
          str_c(
            field_nm, 
            collapse = ",")# %T>% print()
