...
librarian::shelf(
  ...
  # added
  ggrepel, ggtext, readxl, ggnnewscale,
...
cruise_id <- 
  c(
    "WS23061", # TODO:  edit for cruise
    "March 2<sup>nd</sup> - 8<sup>th</sup>, 2023" # TODO: edit for dates
  )
```
...
set.seed(123)
imars_map <- plt +
  annotate(geom = "richtext", y = 28, x = -81.5, 
  ...
  annotate(geom = "richtext", y = 27.85, x = -81.5, 
           label = cruise_id[[2]],
           size = 7,
           fontface = 'bold',
           fill = NA,
...
if (FALSE) {
  fs::dir_create(here("data", "metadata", "cruise_map"))
  ggsave(here("data", "metadata", "cruise_map",
