...
# all
all_loc <- loc_file %>%
  readxl::read_xlsx(sheet = 2, .name_repair = janitor::make_clean_names) %>%
  filter(!(station %in% loca$station))

# zoo
zoo_loc <- loc_file %>%
  readxl::read_xlsx(sheet = 3, .name_repair = janitor::make_clean_names) 
...
# path to map files
path <- here("../zooplankton.test/data/map/")
...
```{r download-map}
# only usefull if don't have access to map file
fs::dir_create(path)
...
if (!file.exists(here(path,"etopo1.nc"))) {
  ...
  griddap(etopo, latitude = exnt[3:4], longitude = exnt[1:2], stride = c(1,1), 
          fields = 'altitude',
          store = disk(path))
...
plt +
 ...
 geom_text_repel(data = loca2, aes(x = lon, y = lat, label = station),
  ...
  scale_color_manual(values = 
                       c("#FF7F24", "#EE3B3B", "#00FF00", "#1E90FF",  "#BA55D3")
                     ) +
  labs(color = NULL) +
  new_scale_color() +
  geom_point(data = zoo_loc, 
             aes(x = lon, y = lat, color = labs),
             shape = 5, size = 2) +
