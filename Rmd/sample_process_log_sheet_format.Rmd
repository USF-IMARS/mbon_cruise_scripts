...
source("./scripts/process_log_sheet.R")
...
meta_all <- 
  ...
  filter(
    year >= 2019
    & !str_detect(cruise_id, c("WS19028|WS19119|WS19210"))) 


# meta <- 
#   meta_all %>% filter(str_detect(cruise_id, cruise))
  ...
  mutate(
    sheets = map2(data, cruise_id,
      ~ tryCatch({
        ...
        cli::cli_alert_danger("Cruise has issue unnesting!")
        cli::cli_alert_danger(sprintf("Error in %s: %s", 
                                      deparse(e[["call"]]), 
                                      e[["message"]])
                              )
        NULL
...
#   try(openXL(meta$sheets[[i]]), silent = TRUE)
# }

```
```{r create-workbook}
wb <- process_log_sheet(.meta = meta$data[[i]], cal = cal, cruise = meta$cruise_id[[i]])
...
# ---- save ----
# saveWorkbook(wb, 
#              here(glue("{cruise}_{meta$year[[1]]}_",
#                        "{min(format(meta$date_mm_dd_yy, '%m'))}_abs.xlsx")), 
#              overwrite = TRUE)

# for (i in seq(nrow(meta))) {
#   try(
#     saveWorkbook(meta$sheets[[i]], 
#                  here(glue("{meta$cruise_id[i]}_{meta$data[[i]]$year[[1]]}_",
#                           "{min(format(meta$data[[i]]$",
#                           "date_mm_dd_yy, '%m'), na.rm = TRUE)}_abs.xlsx")), 
#                  overwrite = TRUE), silent = TRUE)
# }

dir_create(here(cloud_dir, "process_logs"))
for (i in seq(nrow(meta))) {
   file_name <- here(cloud_dir, "process_logs", 
                      glue("{meta$cruise_id[i]}_",
                           "{meta$data[[i]]$year[[1]]}_",
                           "{min(format(meta$data[[i]]$date_mm_dd_yy, '%m'),", 
                           "na.rm = TRUE)}_abs.xlsx"))
   if (!file_exists(file_name)) {
     cli::cli_alert_info("Trying to create file {.file {basename(file_name)}}")
     try(
       saveWorkbook(meta$sheets[[i]],
                    file_name,
                    overwrite = TRUE), silent = TRUE)
   } else {
     cli::cli_alert_info("Ignoring file {.file {basename(file_name)}}")
   }
