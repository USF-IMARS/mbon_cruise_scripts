...
crs_log_exst <- 
  ...
  dir_ls() %>%
  tibble(files = .) %>%
  mutate(cruise_id = str_extract(files, "[A-Z]{2,3}\\d{4,5}"))
  # as.character() %>%
  # basename()  %>%
...
if (update_log) {
  cli_alert_warning(c("You've set update_log = {update_log}. You will be ", 
                      "updating the log!"))
} else {
...
# dir_loc <- here("data", "process_logs", Sys.Date()) # test location
...
for (i in seq(nrow(save_meta))) {
  if (nrow(meta_all) > 0) {
      file_name <- here(dir_loc,
                        glue("{save_meta$cruise_id[i]}_",
                             "{save_meta$data[[i]]$year[[1]]}_",
                             "{min(format(save_meta$data[[i]]$date_mm_dd_yy, ", 
                             "\"%m\"), na.rm = TRUE)}_abs.xlsx"))
  } else if (update_log) {
    file_name <-  
      here(dir_loc, basename(save_meta$files[[i]]))
  }
   if (!file_exists(file_name) | !ignore) {
     cli_alert_info("Trying to create file {.file {basename(file_name)}}")
     try(
       saveWorkbook(save_meta$sheets[[i]],
                    file_name,
                    overwrite = overwrt,
                    returnValue = TRUE), silent = FALSE)
   } else {
