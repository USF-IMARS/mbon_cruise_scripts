...
getSheetNames(prev_f)[2:4]

hplc_check <- 
map(2:4, ~ read.xlsx(prev_f, sheet = .x, startRow = 2)  %>%
           mutate(absent = as.character(absent),
                  notes = as.character(notes))) %>%
  list_rbind() 

meta_format_filt <- 
meta_format %>%
  filter(cruise %in% getSheetNames(prev_f)[2:4]) %>%
  left_join(hplc_check,
