...
Complete the steps outlined in `mbon_imars_cruises/blank_sheets/pre_cruise_tasks.docx`.
...
cruise_id <-
  tibble(
    ...
    start_dates = "September 16 2023", # HERE, month day year
    end_dates   = "September 22 2023", # HERE, month day year
...
loc_file2 <-
  cruise_setup(
    ...
    cruise_location = here("data", "test", "years"),
...
```{r load-locations}
if (is_empty(loc_file2)) {
  cli_alert_info("Cruise Path was not found. Using old file.")
  # sample locations
  loca <-
    loc_file %>%
    readxl::read_xlsx(
  ...
  cli_alert_info("Cruise path was found. Using logsheet.")
  # sample locations
  loca <-
    readxl::read_excel(
      loc_file2,
...
if (!file_exists(here(dirname(loc_file2), basename(sv_arg$file_name)))) {
  file_copy(
        sv_arg$file_name,
        here(dirname(loc_file2)),
        overwrite = TRUE
      )
} else {
